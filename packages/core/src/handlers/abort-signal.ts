 ...
 *
 * Automatic cleanup: when a sender's AbortSignal becomes unreachable (GC'd),
 * the handler automatically sends a `release` so the receiver can clean up
 * its AbortController. This handles the case where a caller passes a signal
 * and then drops it without explicitly calling `controller.abort(COMPLETED)`.
 *
 * @example
 * ```ts
...
> {
  ...
  // Session counter: incremented on disconnect/reconnect so stale finalizer
  // callbacks from a previous session don't corrupt the new session's IDs.
  #session = 0;

  // Sender side: signals we've sent, keyed by ID.
  // Stored as WeakRefs so the signal can be GC'd when user code drops it;
  // #signalFinalizer then sends a release to the receiver automatically.
  #nextId = 1;
  #signalToId = new WeakMap<AbortSignal, number>();
  #sentSignals = new Map<number, WeakRef<AbortSignal>>();
  #abortListeners = new Map<number, () => void>();

  // FinalizationRegistry: fires when a sent signal is GC'd (user dropped all
  // references to the AbortSignal and its AbortController). Sends a release
  // so the receiver can clean up its AbortController.
  // The held value includes a session tag so stale callbacks from before a
  // disconnect/reconnect cycle are ignored (IDs restart from 1 after reconnect).
  #signalFinalizer = new FinalizationRegistry<{id: number; session: number}>(
    ({id, session}) => {
      if (session !== this.#session) return; // stale finalizer, ignore
      if (!this.#abortListeners.has(id)) return; // already cleaned up by abort
  ...
  toWire(signal: AbortSignal, _ctx: ToWireContext): WireAbortSignal {
    ...
    this.#sentSignals.set(id, new WeakRef(signal));

    // Register with FinalizationRegistry so we can send a release if the
    // signal is GC'd (user dropped their AbortController) before aborting.
    // Use the signal itself as the unregister token so we can cancel on abort.
    // The listener below uses a WeakRef to avoid creating a strong cycle that
    // would prevent the signal from being collected.
    this.#signalFinalizer.register(
      signal,
      {id, session: this.#session},
      signal,
    );
    ...
    const listener = () => {
      ...
      // Unregister from finalizer: we've already sent the message ourselves
      if (s !== undefined) {
        this.#signalFinalizer.unregister(s);
      }
      this.#cleanupSent(capturedId);
    };
  ...
  disconnect(): void {
    ...
    // Remove all abort listeners from sent signals and cancel finalizers
    for (const [id, listener] of this.#abortListeners) {
      const signal = this.#sentSignals.get(id)?.deref();
      if (signal !== undefined) {
        signal.removeEventListener('abort', listener);
        this.#signalFinalizer.unregister(signal);
      }
    }
    this.#abortListeners.clear();
    this.#sentSignals.clear();
    // Increment session so any stale finalizers from the old session are
    // ignored after reconnect (IDs restart from 1).
    this.#session++;
  ...
  #cleanupSent(id: number): void {
    this.#abortListeners.delete(id);
    this.#sentSignals.delete(id);
    // #signalToId WeakMap entry will be GC'd automatically
    // FinalizationRegistry is unregistered by the caller before #cleanupSent
  }
